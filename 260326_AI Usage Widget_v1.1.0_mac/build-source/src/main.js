...
function createScraper() {
  ...
  scraperView.webContents.on('did-finish-load', () => {
    setTimeout(extractUsageData, 2500)
  })
...
async function extractUsageData() {
  if (!scraperView) return
    ...
    const data = await scraperView.webContents.executeJavaScript(`
      (function() {
        ...
        const weeklyResetRaw =
          matchIn(weeklyText, /(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*\\s*(?:\\d{1,2}:\\d{2})?|\\d{1,2}[\\/-]\\d{1,2}\\s*\\d{1,2}:\\d{2}|(?:오전|오후)?\\s*\\d{1,2}:\\d{2}/i) ||
          matchIn(pageText, /(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*\\s*(?:\\d{1,2}:\\d{2})?|\\d{1,2}[\\/-]\\d{1,2}\\s*\\d{1,2}:\\d{2}|(?:오전|오후)?\\s*\\d{1,2}:\\d{2}/i)
        ...
        return {
          session: {
            ...
            reset: formatDuration(sessionResetRaw)
          },
          weekly: {
